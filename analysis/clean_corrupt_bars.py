...
ticker_details.csv, and sec_price_data.csv so fetch_market_data.py re-fetches them cleanly.
"""
import pandas as pd

from config.paths import PRICE_BARS, DAILY_BARS, TICKER_DETAILS, PRICES_SEC
BARS_CSV    = PRICE_BARS
DAILY_CSV   = DAILY_BARS
DETAILS_CSV = TICKER_DETAILS
PRICE_CSV   = PRICES_SEC
...
def main():
    ...
    import csv
    # sec_price_data.csv has mixed schema — use csv module
    ...
    print(f"sec_price_data.csv: dropped {dropped} rows, kept {len(kept)}")
